...
<div class="max-w-2xl space-y-6">
    ...
    {#if folderPath.length > 0}
      <div class="rounded-md border border-neutral-200 bg-neutral-50 p-3">
        <div class="flex flex-wrap items-center gap-1 text-sm">
          ...
          {/each}
          <button onclick={chooseFolder} class="ml-auto rounded border border-neutral-300 bg-white px-2 py-0.5 text-xs hover:bg-neutral-100">Watch this folder</button>
        </div>
        <div class="mt-3 space-y-1">
          ...
          {/each}
          {#if !loading && subfolders.length === 0}<p class="text-xs text-neutral-400">No subfolders.</p>{/if}
        </div>
      </div>
    {/if}

    {#if chosenFolder}
      <div class="flex items-center justify-between rounded-md bg-neutral-900 px-3 py-2 text-sm text-white">
  ...
  <div class="space-y-4 rounded-xl border border-neutral-200 bg-white p-6">
    ...
    <label class="block text-sm">
      ...
      <input bind:value={name} placeholder="defaults to the folder name" class="mt-1 w-full rounded-md border border-neutral-300 px-2 py-1.5" />
    </label>
    <label class="block text-sm">
      <span class="text-neutral-500">Destination path (on the Portal host)</span>
      <input bind:value={destinationPath} placeholder="/data/incoming" class="mt-1 w-full rounded-md border border-neutral-300 px-2 py-1.5 font-mono" />
    </label>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      ...
      <label class="block text-sm">
        <span class="text-neutral-500">Path template (optional)</span>
        <input bind:value={pathTemplate} placeholder="{'{project}/{date}/{filename}'}" class="mt-1 w-full rounded-md border border-neutral-300 px-2 py-1.5 font-mono" />
      </label>
    </div>
    <p class="text-xs text-neutral-400">Tokens: {"{filename} {stem} {ext} {project} {folder} {date} {year} {month} {day}"}. Leave blank to keep original names at the destination root.</p>
  ...
  <div class="flex items-center gap-3">
    <button onclick={create} disabled={saving || !chosenFolder} class="rounded-md bg-neutral-900 px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800 disabled:opacity-50">
      {saving ? "Creating…" : "Create rule"}
    </button>
    <a href="/sync-rules" class="text-sm text-neutral-500 hover:text-neutral-900">Cancel</a>
  </div>
</div>
