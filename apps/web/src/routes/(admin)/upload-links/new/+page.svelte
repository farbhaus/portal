  ...
  {:else}
    <div class="space-y-4 rounded-xl border border-neutral-200 bg-white p-6">
      <label class="block text-sm">
        <span class="text-neutral-500">Destination <span class="text-red-500">*</span></span>
        <select bind:value={destinationId} class="mt-1 w-full rounded-md border border-neutral-300 px-2 py-1.5">
      ...
      <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <label class="block text-sm">
          <span class="text-neutral-500">Expires (optional)</span>
        ...
        <label class="block text-sm">
          ...
          <input type="text" bind:value={password} placeholder="leave blank for none" class="mt-1 w-full rounded-md border border-neutral-300 px-2 py-1.5" />
        </label>
        <label class="block text-sm">
          <span class="text-neutral-500">Max file size, GB (optional)</span>
      ...
      <div class="text-sm">
        <label class="flex items-center gap-2">
          ...
          <span class="text-neutral-600">Restrict file types</span>
        </label>
        {#if restrictExtensions}
          ...
          <p class="mt-1 text-xs text-neutral-400">Comma-separated extensions (prefilled with the media preset).</p>
        {/if}
      </div>
    ...
    <div class="space-y-4 rounded-xl border border-neutral-200 bg-white p-6">
      <h2 class="font-medium">Branding overrides <span class="text-xs font-normal text-neutral-400">(optional — falls back to the destination)</span></h2>
      <label class="block text-sm">
        <span class="text-neutral-500">Display name</span>
        <input bind:value={brandDisplayName} placeholder="e.g. Send me your footage" class="mt-1 w-full rounded-md border border-neutral-300 px-2 py-1.5" />
      </label>
      <label class="block text-sm">
    ...
    <div class="flex items-center gap-3">
      <button onclick={create} disabled={saving} class="rounded-md bg-neutral-900 px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800 disabled:opacity-50">
