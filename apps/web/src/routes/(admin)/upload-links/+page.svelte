...
<div class="max-w-5xl space-y-6">
  <div class="flex items-center justify-between">
    <div>
      ...
      <p class="text-sm text-neutral-500">Branded links for collaborators to drop files into a destination.</p>
    </div>
    <a href="/upload-links/new" class="rounded-md bg-neutral-900 px-4 py-2 text-sm font-medium text-white hover:bg-neutral-800">
      New link
    </a>
  </div>
  ...
  {#if data.links.length === 0}
    <div class="rounded-xl border border-dashed border-neutral-300 bg-white p-10 text-center">
      <p class="text-sm text-neutral-500">No upload links yet.</p>
      <a href="/upload-links/new" class="mt-2 inline-block text-sm font-medium underline">Create one</a>
    </div>
  {:else}
      ...
      <table class="w-full text-sm">
        <thead class="border-b border-neutral-200 text-left text-neutral-500">
          <tr>
            ...
            <th class="px-4 py-2.5 font-medium">Protections</th>
        ...
        <tbody>
          {#each data.links as link (link.id)}
            <tr class="border-b border-neutral-100 align-top last:border-0">
              ...
              <td class="px-4 py-3 text-neutral-600">{link.destination_name}</td>
              <td class="px-4 py-3">
              ...
              <td class="px-4 py-3 text-xs text-neutral-500">
                ...
                {#if !link.password_protected && !link.expires_at && !link.allowed_extensions}—{/if}
              </td>
              <td class="px-4 py-3 text-right whitespace-nowrap">
                <a href={link.public_url} target="_blank" rel="noopener" class="text-neutral-600 hover:text-neutral-900">Open</a>
                <a href="/upload-links/{link.id}" class="ml-3 text-neutral-600 hover:text-neutral-900">Edit</a>
                {#if link.state === "ok"}
