...
<div class="space-y-5">
  <PageHeader title="Links" subtitle="Branded links to send files out to recipients (no Frame.io account needed).">
    {#snippet actions()}
      <Button href="/download-links/new">New download link</Button>
    {/snippet}
  </PageHeader>
  ...
  {#if data.links.length === 0}
    <EmptyState message="No download links yet.">
      <Button href="/download-links/new" size="sm">Create one</Button>
    </EmptyState>
  {:else}
    <Table columns={["Link", "Source", "Status", "Downloads", { label: "", class: "text-right" }]}>
      {#each data.links as link (link.id)}
        <tr class="align-top">
          ...
          <td class="px-4 py-3 text-muted">{sourceLabel(link.source)}</td>
          ...
          <td class="px-4 py-3 text-right whitespace-nowrap">
            ...
            <a href="/download-links/{link.id}" class="ml-3 text-muted hover:text-text">Edit</a>
            {#if link.state === "ok"}
